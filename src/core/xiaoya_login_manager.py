...
该模块实现了通过HTTP请求模拟浏览器操作，完成武汉理工大学小雅平台的登录流程。
...
class XiaoyaLoginManager:
    ...
    def _process_cas_login(self, cas_login_url) -> str:
        ...
        try:
            ...
            response = self.session.get(cas_login_url, headers=headers, allow_redirects=False)
    ...
    def _get_authorization_code(self, redirect_url):
        ...
        try:
            ...
            # 请求授权重定向API，不允许自动重定向
            redirect_response = self.session.get(redirect_api_url, headers=redirect_headers, allow_redirects=False)
